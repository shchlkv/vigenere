...
#продумать, как заглавные буквы менять на строчные
#ввести проверку на ввод некорректных символов или латиницы
...
for word in vigenere_file_text:
        print(z , word) #читаем стороку из файла и присваиваем ее переменной word.
        #Если нет лишних символов, но фраза разбита на строки, то будет зашифрована последняя фраза
        #нужно склеить фразу
        z+=1

# если в тексте есть заглавные буквы - преобразуем в строчные
...
for letter in word:
        if word[i] not in alphabet:
               то этот символ удаляется и word должно смещаться на символ влево
                word.replace(" ", ""))
   '''            

#разобраться, как удалить пробелы и прочие символы из текста
# как идея - если нет символа в списке alphabet, то он удаляется.


'''
предыдущий блок делает все тоже, что и этот    
vigenere_file_text=open('/home/ark/vigenere_file_text.txt', 'r')
for word in vigenere_file_text.readlines():
        print("2  ", word) # выводятся обе строки файла, но шифруется только вторая

'''

#print("3 ", vigenere_file_text.read())
